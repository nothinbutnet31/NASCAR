...
function generateWeeklyRecap() {
  ...
  if (topDriversWithOver30.length > 0) {
    ...
    topDriversWithOver30.forEach(driver => {
      topPerformersHTML += `<li>${driver.driver} - ${driver.totalPoints} points</li>`;
    });
    topPerformersHTML += "</ul>";
  } else {
    topPerformersHTML += "<p>No standout performers this week.</p>";
  }

  recapHTML += topPerformersHTML;

  // Display standings changes
  let standingsChangesHTML = "<h3>Standings Changes:</h3>";
  let standingsChangeText = "";
  sortedTeams.forEach((team, index) => {
    standingsChangeText += `<p>${index + 1}. ${team[0]} - ${team[1]} points</p>`;
  });

  standingsChangesHTML += standingsChangeText;
  recapHTML += standingsChangesHTML;
